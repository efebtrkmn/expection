...
export class ClientStatementService {
  ...
  /**
   * Müşterinin cari ekstresini Puppeteer ile PDF olarak üretir.
   * Puppeteer lazy-initialize edilir (startup süresini etkilememesi için).
   */
  async generateStatementPdf(contactId: string, tenantId: string): Promise<Buffer> {
    ...
    try {
      ...
      // Puppeteer yüklü değilse plain HTML buffer döndür
      return Buffer.from(html);
    }
  }

  private buildStatementHtml(data: {
    customer: any;
    tenant: any;
    invoices: any[];
    generatedAt: Date;
    ...
    const totalDebt = data.invoices
      .filter(i => [InvoiceStatus.ISSUED, InvoiceStatus.SENT, InvoiceStatus.OVERDUE].includes(i.status))
